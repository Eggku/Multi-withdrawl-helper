...
*   支持多家主流交易所 (当前已配置: Binance, OKX)。
*   通过导入Excel/CSV文件批量执行提币到多个地址。
*   可自定义每次提币的随机金额范围和两次提币之间的随机等待间隔。
*   提供详细的操作日志记录和交易所提币历史查询功能。
*   内置地址格式验证功能，减少错误。
*   用户友好的图形界面，支持深色主题。
...
2.  **API密钥配置**:
    *   点击主界面顶部的 **「设置」** 按钮。
    *   为每个计划使用的交易所填入有效的 **API Key**, **API Secret** (OKX可能还需要 **Passphrase**)。
    *   **重要**: 确保API密钥具有"允许提现"的权限。为安全起见，强烈建议为您的API密钥配置IP白名单，仅允许您的常用IP地址访问。
...
2.  **导入地址**: 点击「导入地址」，选择包含提币地址的 Excel (`.xlsx`) 或 CSV (`.csv`) 文件。
    *   文件应至少包含一列 `address` (地址)。可选地，可以包含 `label` (备注/标签) 列。
...
*   **测试网络/模拟盘**: 如果交易所支持，请先在其测试网络或模拟盘环境中测试本工具。 (OKX支持模拟盘设置)

## 更新说明 (v{APP_VERSION})

*   (此版本的主要更新、修复或新功能，请在此处填写)
    *   例如: 修复了OKX交易所EVM地址提币时label处理错误的问题。
    *   例如: 优化了日志系统，减少了不必要的DEBUG信息输出。
    *   例如: 增强了UI反馈，提升了用户体验。
